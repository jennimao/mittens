...
# INITIALIZE ARDUINO
########################
board = Arduino('/dev/cu.usbserial-14240')
      ...
      if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
          ...
          if (lock): 
            ...
            # check if any points are on the cube  
            """
            for finger_key in finger_landmarks_dict.keys():
              pin = finger_landmarks_dict[finger_key]
              print("HELLO")
              if projected_points:
                print("projected")
                if on_Cube(projected_points, hand_landmarks.landmark[finger_key].x, hand_landmarks.landmark[finger_key].y):
                    print("TOUCHING CUBE " + str(finger_key))
